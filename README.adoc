...
You can find the release history assets in the `docs/releasehx/` directory, which contains a configuration file and RHYML files that are auto-generated and then manually edited to produce the GitHub link:https://github.com/DocOps/issuer/releases[release announcements].

. Draft RHYML draft file from the online GitHub Issues for the given version:
+
[.prompt,subs=+attributes]
 bundle exec rhx {this_prod_vrsn} --yaml docs/releasehx/release-{this_prod_vrsn}.yml --config docs/releasehx/config.yml
